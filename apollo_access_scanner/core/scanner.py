...
class ApolloScanner:
    ...
    def run_single_provider(self, provider_name: str, yes: bool = False, **kwargs: Any) -> None:
        """Run scan for a single provider with credentials supplied as kwargs.

        Pass yes=True to skip the confirmation prompt (for automation / non-TTY contexts).
        """
        ...
        if not yes and not typer.confirm(f"Proceed with {provider_name} scan?", default=True):
            self.console.print("[yellow]Scan cancelled.[/yellow]")
            return
